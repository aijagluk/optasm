...
GOpcodeROR::GOpcodeROR(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeROR::initUI(void){
    ...
    QLabel* plblROR = new QLabel("<h1><strong>ror</strong></h1>");
    QLabel* plblComma = new QLabel("<h1><strong>,</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblROR);
...
QString GOpcodeROR::sourceCode(void){
    QString str;
    str.append("ror ");
    str.append(m_strOp1);
    str.append(", ");
...
void GOpcodeROR::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeROR::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

int GOpcodeROR::calcEA(QString str){
...
void GOpcodeROR::analyse(void){
    ...
    if((oper1 == BASE_REGISTER)){
        ...
        if(m_strOp2.contains(rxpCL)){
            ...
            m_nTicks = 0;
            qDebug() << "m_nTicks = 0; opcode_ror.cpp; \"analyse\" function\n";
        }
        ...
        if(m_strOp1.contains(rxp2Bytes)){
            ...
            qDebug() << "m_nBytes = 0; opcode_ror.cpp; \"analyse\" function\n";
        }
    }
