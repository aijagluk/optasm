...
GOpcodeBTC::GOpcodeBTC(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeBTC::initUI(void){
    ...
    QLabel* plblBTC = new QLabel("<h1><strong>btc</strong></h1>");
    QLabel* plblComma = new QLabel("<h1><strong>,</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblBTC);
...
QString GOpcodeBTC::sourceCode(void){
    QString str;
    str.append("btc ");
    str.append(m_strOp1);
    str.append(", ");
...
void GOpcodeBTC::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeBTC::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

void GOpcodeBTC::analyse(void){
    //заглушка
    m_nTicks = 0;
    m_nBytes = 0;
}
