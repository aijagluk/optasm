...
void GOpcodeNEG::clearSource(void){
    m_pleOp->clear();
}

void GOpcodeNEG::saveData(void){
    setOp1(m_pleOp->text());
}

int GOpcodeNEG::calcEA(QString str){
...
void GOpcodeNEG::analyse(void){
    ...
    if(oper1 == BASE_REGISTER){
        ...
        if(m_strOp1.contains(rxpByte)){
            ...
            qDebug() << "m_nBytes = 0; opcode_neg.cpp \"analyse\" function\n";
        }
    }
}
