...
GOpcodeSAL::GOpcodeSAL(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeSAL::initUI(void){
    ...
    QLabel* plblSAL = new QLabel("<h1><strong>sal</strong></h1>");
    QLabel* plblComma = new QLabel("<h1><strong>,</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblSAL);
...
QString GOpcodeSAL::sourceCode(void){
    QString str;
    str.append("sal ");
    str.append(m_strOp1);
    str.append(", ");
...
void GOpcodeSAL::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeSAL::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

int GOpcodeSAL::calcEA(QString str){
...
void GOpcodeSAL::analyse(void){
    ...
    if((oper1 == BASE_REGISTER)){
        ...
        if(m_strOp2.contains(rxpCL)){
            ...
            m_nTicks = 0;
            qDebug() << "m_nTicks = 0; opcode_sal.cpp; \"analyse\" function\n";
        }
        ...
        if(m_strOp1.contains(rxp2Bytes)){
            ...
            qDebug() << "m_nBytes = 0; opcode_sal.cpp; \"analyse\" function\n";
        }
    }
