...
GOpcodeOR::GOpcodeOR(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeOR::initUI(void){
    ...
    QLabel* plblOR = new QLabel("<h1><strong>or</strong></h1>");
    QLabel* plblComma = new QLabel("<h1><strong>,</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblOR);
    phblGeneral->addWidget(m_pleOp1);
    phblGeneral->addWidget(plblComma);
    phblGeneral->addWidget(m_pleOp2);
    phblGeneral->addStretch();

    QString info_str = "Реализует поразрядную булевую функцию ИЛИ (дизъюнкцию). Второй операнд играет роль маски. "
            ...
            "Флажки SF, ZF и PF устанавливаются в зависимости от полученного результата (по правилам арифметических "
...
QString GOpcodeOR::sourceCode(void){
    QString str;
    str.append("or ");
    str.append(m_strOp1);
    str.append(", ");
...
void GOpcodeOR::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeOR::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

int GOpcodeOR::calcEA(QString str){
