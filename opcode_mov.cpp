...
void GOpcodeMOV::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeMOV::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

int GOpcodeMOV::calcEA(QString str){
...
void GOpcodeMOV::analyse(void){
    ...
    if((oper1 == BASE_REGISTER && oper2 == BASE_REGISTER) || (oper1 == ACCUMULATOR && oper2 == ACCUMULATOR)){
        m_nTicks = 2;
        m_nBytes = 2;
    }
    ...
    else if( (oper1 == BASE_REGISTER || oper1 == ACCUMULATOR) && oper2 == CONSTANT_VALUE){
        m_nTicks = 4;
    ...
    else if(oper1 == ACCUMULATOR && oper2 == MEMORY){
        m_nTicks = 10;
        m_nBytes = 3;
    }
    else if(oper1 == MEMORY && oper2 == ACCUMULATOR){
        m_nTicks = 10;
        m_nBytes = 3;
    }
    else if(oper1 == SEGMENT_REGISTER_WITHOUT_CS && oper2 == MEMORY){
        EA_ticks = calcEA(m_strOp2);
        if(EA_ticks == 0){
            qDebug() << "EA == 0\n";
        }
    ...
    else if(oper1 == SEGMENT_REGISTER_WITHOUT_CS && oper2 == BASE_REGISTER){
        m_nTicks = 8;
    ...
    else if(oper1 == MEMORY && oper2 == SEGMENT_REGISTER){
        EA_ticks = calcEA(m_strOp1);
        if(EA_ticks == 0){
            qDebug() << "EA == 0\n";
        }
    ...
    else if(oper1 == BASE_REGISTER && oper2 == SEGMENT_REGISTER){
        m_nTicks = 9;
        ...
        qDebug() << "MOV: command is not recognize!\n";
    }
}
