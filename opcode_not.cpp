...
void GOpcodeNOT::initUI(void){
    m_pleOp = new QLineEdit();

    m_pleOp->setAlignment(Qt::AlignCenter);

    QLabel* plblNOT = new QLabel("<h1><strong>not</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblNOT);
...
void GOpcodeNOT::clearSource(void){
    m_pleOp->clear();
}

void GOpcodeNOT::saveData(void){
    setOp1(m_pleOp->text());
}

int GOpcodeNOT::calcEA(QString str){
