...
GOpcodeREPNZ_SCAS::GOpcodeREPNZ_SCAS(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeREPNZ_SCAS::initUI(void){
    m_pleOp1 = new QLineEdit();

    m_pleOp1->setAlignment(Qt::AlignCenter);

    QLabel* plblREPNZ_SCAS = new QLabel("<h1><strong>repnz scas</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblREPNZ_SCAS);
...
QString GOpcodeREPNZ_SCAS::sourceCode(void){
    QString str;
    str.append("repnz scas ");
    str.append(m_strOp1);

    return(str);
}

void GOpcodeREPNZ_SCAS::clearSource(void){
    m_pleOp1->clear();
}

void GOpcodeREPNZ_SCAS::saveData(void){
    setOp1(m_pleOp1->text());
}

void GOpcodeREPNZ_SCAS::analyse(void){
