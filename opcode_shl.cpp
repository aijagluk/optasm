...
GOpcodeSHL::GOpcodeSHL(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeSHL::initUI(void){
    ...
    QLabel* plblSHL = new QLabel("<h1><strong>shl</strong></h1>");
    QLabel* plblComma = new QLabel("<h1><strong>,</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblSHL);
...
QString GOpcodeSHL::sourceCode(void){
    QString str;
    str.append("shl ");
    str.append(m_strOp1);
    str.append(", ");
...
void GOpcodeSHL::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeSHL::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

int GOpcodeSHL::calcEA(QString str){
...
void GOpcodeSHL::analyse(void){
    ...
    if((oper1 == BASE_REGISTER)){
        ...
        if(m_strOp2.contains(rxpCL)){
            ...
            m_nTicks = 0;
            qDebug() << "m_nTicks = 0; opcode_shl.cpp; \"analyse\" function\n";
        }
        ...
        if(m_strOp1.contains(rxp2Bytes)){
            ...
            qDebug() << "m_nBytes = 0; opcode_shl.cpp; \"analyse\" function\n";
        }
    }
