...
GOpcodeOUT::GOpcodeOUT(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeOUT::initUI(void){
    ...
    QLabel* plblOUT = new QLabel("<h1><strong>out</strong></h1>");
    QLabel* plblComma = new QLabel("<h1><strong>,</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblOUT);
...
QString GOpcodeOUT::sourceCode(void){
    QString str;
    str.append("out ");
    str.append(m_strOp1);
    str.append(", ");
...
void GOpcodeOUT::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeOUT::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

void GOpcodeOUT::analyse(void){
    ...
    if(oper1 == CONSTANT_VALUE && oper2 == ACCUMULATOR){
        m_nTicks = 10;
        m_nBytes = 2;
    }
    else if(oper1 == BASE_REGISTER && oper2 == ACCUMULATOR){
        m_nTicks = 8;

        if(m_strOp1.contains(rxpDX)){
            m_nBytes = 1;
        }
        else{
            m_nBytes = 0;
            qDebug() << "Error: opcode_out.cpp \"analyse\" function";
        }
    }
    else{
