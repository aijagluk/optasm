...
void GOpcodeJMP::clearSource(void){
    m_pleOp->clear();
}

void GOpcodeJMP::saveData(void){
    setOp1(m_pleOp->text());
}

int GOpcodeJMP::calcEA(QString str){
...
void GOpcodeJMP::analyse(void){
    ...
    if(m_strOp1.contains(rxpLabel)){
        ...
        if(m_strOp1.contains(rxpByte)){
            ...
            qDebug() << "m_nBytes = 0; opcode_jmp.cpp \"analyse\" function\n";
        }
    }
    else{
        qDebug() << "Bad parameter. opcode_jmp.cpp; \"analyse\" function\n";
    }
}
