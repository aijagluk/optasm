...
GOpcodeSHR::GOpcodeSHR(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeSHR::initUI(void){
    ...
    QLabel* plblSHR = new QLabel("<h1><strong>shr</strong></h1>");
    QLabel* plblComma = new QLabel("<h1><strong>,</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblSHR);
...
QString GOpcodeSHR::sourceCode(void){
    QString str;
    str.append("shr ");
    str.append(m_strOp1);
    str.append(", ");
...
void GOpcodeSHR::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeSHR::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

int GOpcodeSHR::calcEA(QString str){
...
void GOpcodeSHR::analyse(void){
    ...
    if((oper1 == BASE_REGISTER)){
        ...
        if(m_strOp2.contains(rxpCL)){
            ...
            m_nTicks = 0;
            qDebug() << "m_nTicks = 0; opcode_shr.cpp; \"analyse\" function\n";
        }
        ...
        if(m_strOp1.contains(rxp2Bytes)){
            ...
            qDebug() << "m_nBytes = 0; opcode_shr.cpp; \"analyse\" function\n";
        }
    }
