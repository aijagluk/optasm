...
void GOpcodeCALL::initUI(void){
    m_pleOp = new QLineEdit();

    m_pleOp->setAlignment(Qt::AlignCenter);

    QLabel* plblCALL = new QLabel("<h1><strong>call</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblCALL);
...
void GOpcodeCALL::clearSource(void){
    m_pleOp->clear();
}

void GOpcodeCALL::saveData(void){
    setOp1(m_pleOp->text());
}

int GOpcodeCALL::calcEA(QString str){
...
void GOpcodeCALL::analyse(void){
    ...
    if(oper1 == BASE_REGISTER){
        ...
        if(m_strOp1.contains(rxpByte)){
            ...
            qDebug() << "m_nBytes = 0; opcode_call.cpp \"analyse\" function\n";
        }
    }
