...
GOpcodePUSH::GOpcodePUSH(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodePUSH::initUI(void){
    m_pleOp = new QLineEdit();

    m_pleOp->setAlignment(Qt::AlignCenter);

    QLabel* plblPUSH = new QLabel("<h1><strong>push</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblPUSH);
...
QString GOpcodePUSH::sourceCode(void){
    QString str;
    str.append("push ");
    str.append(m_strOp1);
...
void GOpcodePUSH::clearSource(void){
    m_pleOp->clear();
}

void GOpcodePUSH::saveData(void){
    setOp1(m_pleOp->text());
}

int GOpcodePUSH::calcEA(QString str){
...
void GOpcodePUSH::analyse(void){
    ...
    if(oper1 == BASE_REGISTER){
        ...
        if(m_strOp1.contains(rxpByte)){
            ...
            qDebug() << "m_nBytes = 0; opcode_push.cpp \"analyse\" function";
        }
    }
}
