...
GOpcodeAND::GOpcodeAND(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeAND::initUI(void){
    ...
    QLabel* plblAND = new QLabel("<h1><strong>and</strong></h1>");
    QLabel* plblComma = new QLabel("<h1><strong>,</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblAND);
    phblGeneral->addWidget(m_pleOp1);
    phblGeneral->addWidget(plblComma);
    phblGeneral->addWidget(m_pleOp2);
    phblGeneral->addStretch();

    QString info_str = "Реализует поразрядно булевую функцию И (конъюнкцию). Второй операнд выполняет "
            "роль маски. Результат заносится на место первого операнда.\n"
            ...
            "Используется для перевода в нулевое состояние тех бит первого операнда, которые определяются маской, "
            "содержащей нули в сбрасываемых битах и единицы в остальных.";
...
QString GOpcodeAND::sourceCode(void){
    QString str;
    str.append("and ");
    str.append(m_strOp1);
    str.append(", ");
...
void GOpcodeAND::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeAND::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

int GOpcodeAND::calcEA(QString str){
