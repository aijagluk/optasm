...
GOpcodeRCL::GOpcodeRCL(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeRCL::initUI(void){
    ...
    QLabel* plblRCL = new QLabel("<h1><strong>rcl</strong></h1>");
    QLabel* plblComma = new QLabel("<h1><strong>,</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblRCL);
...
QString GOpcodeRCL::sourceCode(void){
    QString str;
    str.append("rcl ");
    str.append(m_strOp1);
    str.append(", ");
...
void GOpcodeRCL::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeRCL::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

int GOpcodeRCL::calcEA(QString str){
...
void GOpcodeRCL::analyse(void){
    ...
    if((oper1 == BASE_REGISTER)){
        ...
        if(m_strOp2.contains(rxpCL)){
            ...
            m_nTicks = 0;
            qDebug() << "m_nTicks = 0; opcode_rcl.cpp; \"analyse\" function\n";
        }
        ...
        if(m_strOp1.contains(rxp2Bytes)){
            ...
            qDebug() << "m_nBytes = 0; opcode_rcl.cpp; \"analyse\" function\n";
        }
    }
