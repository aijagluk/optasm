...
GOpcodeROL::GOpcodeROL(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeROL::initUI(void){
    ...
    QLabel* plblROL = new QLabel("<h1><strong>rol</strong></h1>");
    QLabel* plblComma = new QLabel("<h1><strong>,</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblROL);
...
QString GOpcodeROL::sourceCode(void){
    QString str;
    str.append("rol ");
    str.append(m_strOp1);
    str.append(", ");
...
void GOpcodeROL::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeROL::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

int GOpcodeROL::calcEA(QString str){
...
void GOpcodeROL::analyse(void){
    ...
    if((oper1 == BASE_REGISTER)){
        ...
        if(m_strOp2.contains(rxpCL)){
            ...
            m_nTicks = 0;
            qDebug() << "m_nTicks = 0; opcode_rol.cpp; \"analyse\" function\n";
        }
        ...
        if(m_strOp1.contains(rxp2Bytes)){
            ...
            qDebug() << "m_nBytes = 0; opcode_rol.cpp; \"analyse\" function\n";
        }
    }
