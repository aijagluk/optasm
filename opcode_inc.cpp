...
void GOpcodeINC::initUI(void){
    m_pleOp = new QLineEdit();

    m_pleOp->setAlignment(Qt::AlignCenter);

    QLabel* plblINC = new QLabel("<h1><strong>inc</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblINC);
...
void GOpcodeINC::clearSource(void){
    m_pleOp->clear();
}

void GOpcodeINC::saveData(void){
    setOp1(m_pleOp->text());
}

int GOpcodeINC::calcEA(QString str){
...
void GOpcodeINC::analyse(void){
    ...
    if(oper1 == BASE_REGISTER){
        ...
        if(m_strOp1.contains(rxpByte)){
            ...
            qDebug() << "m_nBytes = 0; opcode_inc.cpp \"analyse\" function\n";
        }
    }
}
