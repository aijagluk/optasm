...
void GOpcodeIMUL::initUI(void){
    m_pleOp = new QLineEdit();

    m_pleOp->setAlignment(Qt::AlignCenter);

    QLabel* plblIMUL = new QLabel("<h1><strong>imul</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblIMUL);
...
void GOpcodeIMUL::clearSource(void){
    m_pleOp->clear();
}

void GOpcodeIMUL::saveData(void){
    setOp1(m_pleOp->text());
}

int GOpcodeIMUL::calcEA(QString str){
...
void GOpcodeIMUL::analyse(void){
    ...
    if(oper1 == BASE_REGISTER){
        ...
        if(m_strOp1.contains(rxp2Bytes)){
            ...
            qDebug() << "Error: bad parameter. opcode_imul.cpp \"analyse\" function\n";
        }
        ...
        if(m_strOp1.contains(rxpByte)){
            ...
            qDebug() << "m_nBytes = 0; opcode_imul.cpp \"analyse\" function\n";
        }
    }
}
