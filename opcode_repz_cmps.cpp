...
GOpcodeREPZ_CMPS::GOpcodeREPZ_CMPS(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeREPZ_CMPS::initUI(void){
    m_pleOp1 = new QLineEdit();

    m_pleOp1->setAlignment(Qt::AlignCenter);

    QLabel* plblREPZ_CMPS = new QLabel("<h1><strong>repz cmps</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblREPZ_CMPS);
...
QString GOpcodeREPZ_CMPS::sourceCode(void){
    QString str;
    str.append("repz cmps ");
    str.append(m_strOp1);

    return(str);
}

void GOpcodeREPZ_CMPS::clearSource(void){
    m_pleOp1->clear();
}

void GOpcodeREPZ_CMPS::saveData(void){
    setOp1(m_pleOp1->text());
}

void GOpcodeREPZ_CMPS::analyse(void){
