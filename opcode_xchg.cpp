...
GOpcodeXCHG::GOpcodeXCHG(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeXCHG::initUI(void){
    ...
    QLabel* plblXCHG = new QLabel("<h1><strong>xchg</strong></h1>");
    QLabel* plblComma = new QLabel("<h1><strong>,</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblXCHG);
...
QString GOpcodeXCHG::sourceCode(void){
    QString str;
    str.append("xchg ");
    str.append(m_strOp1);
    str.append(", ");
...
void GOpcodeXCHG::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeXCHG::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

int GOpcodeXCHG::calcEA(QString str){
...
void GOpcodeXCHG::analyse(void){
    ...
    if(oper1 == BASE_REGISTER && oper2 == ACCUMULATOR){
        m_nTicks = 1; // none at all!
        m_nBytes = 1;
    }
    else if( (oper1 == BASE_REGISTER || oper1 == ACCUMULATOR) && oper2 == BASE_REGISTER){
        m_nTicks = 4;

        if(m_strOp1.contains(rxpByte) && m_strOp2.contains(rxpByte)){
            m_nBytes = 2;
        }
        else if(m_strOp1.contains(rxp2Bytes) && m_strOp2.contains(rxp2Bytes)){
            m_nBytes = 4;
        }
        else{
        ...
        qDebug() << "XCHG: command is not recognize!\n";
    }
}
