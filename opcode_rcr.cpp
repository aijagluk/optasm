...
GOpcodeRCR::GOpcodeRCR(QWidget *parent)
    :QWidget(parent){

    initUI();
}

void GOpcodeRCR::initUI(void){
    ...
    QLabel* plblRCR = new QLabel("<h1><strong>rcr</strong></h1>");
    QLabel* plblComma = new QLabel("<h1><strong>,</strong></h1>");

    QHBoxLayout* phblGeneral = new QHBoxLayout();
    phblGeneral->addStretch();
    phblGeneral->addWidget(plblRCR);
...
QString GOpcodeRCR::sourceCode(void){
    QString str;
    str.append("rcr ");
    str.append(m_strOp1);
    str.append(", ");
...
void GOpcodeRCR::clearSource(void){
    m_pleOp1->clear();
    m_pleOp2->clear();
}

void GOpcodeRCR::saveData(void){
    setOp1(m_pleOp1->text());
    setOp2(m_pleOp2->text());
}

int GOpcodeRCR::calcEA(QString str){
...
void GOpcodeRCR::analyse(void){
    ...
    if((oper1 == BASE_REGISTER)){
        ...
        if(m_strOp2.contains(rxpCL)){
            ...
            m_nTicks = 0;
            qDebug() << "m_nTicks = 0; opcode_rcr.cpp; \"analyse\" function\n";
        }
        ...
        if(m_strOp1.contains(rxp2Bytes)){
            ...
            qDebug() << "m_nBytes = 0; opcode_rcr.cpp; \"analyse\" function\n";
        }
    }
